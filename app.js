...
function renderHistory() {
  ...
  if (chatHistory.length === 0) {
    appendAssistantBubble("Hey, ich bin Jarvis. Frag mich was, oder tippe auf das Mikrofon, um zu sprechen. Ich kann bei Bedarf im Internet nachsehen, wenn du etwas Aktuelles wissen willst.", false);
    return;
  }
  chatHistory.forEach(msg => {
    if (msg.role === 'user') {
      appendUserBubble(msg.displayText || msg.content, false);
    } else {
      appendAssistantBubble(msg.displayText || msg.content, false);
    }
  });
  scrollToBottom();
}
...
function appendAssistantBubble(text, animate = true, isError = false) {
  const div = document.createElement('div');
  div.className = 'msg assistant';
  div.innerHTML = `<div class="bubble${isError ? ' error' : ''}"></div>`;
  div.querySelector('.bubble').textContent = text;
  chatLog.appendChild(div);
  scrollToBottom();
  return div;
...
async function sendMessage(text) {
  ...
  if (result.error) {
    setStatus('error');
    appendAssistantBubble(result.text, true, true);
    setTimeout(() => setStatus('idle'), 2500);
  } else {
    setStatus('idle');
    appendAssistantBubble(result.text, true);
...
micBtn.addEventListener('click', () => {
  ...
  if (!recognition) {
    appendAssistantBubble("Spracherkennung wird auf diesem Gerät/Browser nicht unterstützt. Nutze am besten Safari auf dem iPhone.", true, true);
    return;
  }
...
async function renderWatchlist() {
  if (watchlist.length === 0) {
    watchlistContainer.innerHTML = `<div class="empty-state">Noch nichts auf der Liste.<br>Füg oben einen Coin (z.B. "bitcoin") oder eine Aktie (z.B. "AAPL") hinzu.</div>`;
    return;
  }
